...
export async function fetchTransactionWithRetry(
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (isLastAttempt) {
        throw new Error(
          `Failed to fetch transaction ${txid} after ${maxRetries} attempts: ${errorMessage}`,
        );
      }
      console.log(
        `Transaction fetch failed (attempt ${attempt + 1}/${maxRetries}), retrying in ${delayMs}ms...`,
      );
