...
describe('prepareCommitTx (integration)', () => {
  ...
  it('returns a valid CommitPsbtResult with sufficient funds', async () => {
    ...
    expect(result.taprootRevealScript).toEqual(mockInscriptionData.taprootRevealScript);
    expect(result.taprootRevealValue).toBe(mockInscriptionData.taprootRevealValue);
    ...
    expect(result.inscriptionScript).toEqual(mockInscriptionData.inscriptionScript);
  });
  ...
  it('throws InsufficientFundsError if not enough sats', async () => {
    ...
    await expect(
      prepareCommitTx('bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh', 'bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh', '02'.padEnd(66, '0'), 1)
    ).rejects.toThrow(InsufficientFundsError);
  });

  it('throws InvalidParametersError for P2SH address without public key', async () => {
    await expect(
      prepareCommitTx('3QJmV3qfvL9SuYo34YihAf3sRCW3qSinyC', 'bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh', '02'.padEnd(66, '0'), 1)
