...
import { createInscriptionScript, estimateRevealFeeInSats } from "./inscription-utils";
import { DUST_LIMIT, DEFAULT_FEE_RATE } from "@/features/bitcoin/lib/constants";
import { generateCompositeImageBuffer } from "@/features/bitcoin/lib/image-gen/generate-image";
...
export async function generateInscriptionData(
  ...
  const imageBuffer = await generateCompositeImageBuffer(ordinalsAddress, sessionId);
  ...
  if (userPubKey.length === 32) {
    ...
    userXOnlyPubkey = secp256k1.xOnlyPointFromPoint(Uint8Array.from(userPubKey));
  }

  const inscriptionScript = createInscriptionScript(userXOnlyPubkey, contentType, content);
  ...
  const controlBlock = taprootPayment.witness[taprootPayment.witness.length - 1];

  const revealFee = estimateRevealFeeInSats(content.length, feeRate);
