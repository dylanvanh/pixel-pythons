...
const DEFAULT_UTXO_RETRY_COUNT = 20;
const UTXO_RETRY_DELAY_500_MILLISECONDS_MS = 500;

export async function getCleanPaymentUtxos(
  userPaymentAddress: string,
  excludeSpentUtxos?: Set<string>,
  maxRetries = DEFAULT_UTXO_RETRY_COUNT,
  delayMs = UTXO_RETRY_DELAY_500_MILLISECONDS_MS,
) {
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    const availableUtxos = rawUtxos
      ...
      .map((utxo) => {
        const [txid, outputIndexText] = utxo.outpoint.split(":");
        const vout = parseInt(outputIndexText, 10);
...
export async function getAvailablePaymentUtxos(
  ...
  const spentUtxoOutpoints = new Set(commitTx.vin.map((input) => `${input.txid}:${input.vout}`));

  return getCleanPaymentUtxos(userPaymentAddress, spentUtxoOutpoints, maxRetries, delayMs);
}

async function fetchTransactionWithRetry(txid: string, maxRetries: number, delayMs: number) {
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
