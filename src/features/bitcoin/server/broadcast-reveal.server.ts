import { bitcoin } from "@/features/bitcoin/lib/bitcoin/core/bitcoin-config"; // Ensure this path is correct
...
import { BroadcastRevealRequestSchema } from "@/features/bitcoin/lib/zod-types/broadcast-reveal";
...
export const POST = withErrorHandling(async (request: Request) => {
  ...
  try {
    ...
    console.error(error);
    throw new AppError("Failed to extract transaction data", ErrorCode.INVALID_PARAMETERS);
  }
  ...
  try {
    ...
    throw new AppError("Failed to broadcast, please try again", ErrorCode.BROADCAST_FAILED);
  }
