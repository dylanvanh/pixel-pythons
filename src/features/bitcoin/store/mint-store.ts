...
import { postInternalApi } from "@/features/bitcoin/lib/internal-api-client";
import { handleError } from "@/features/bitcoin/lib/error/handlers/error-handler";
import { bitcoin } from "@/features/bitcoin/lib/bitcoin/core/bitcoin-config";
import { PrepareCommitRequest } from "@/features/bitcoin/lib/zod-types/commit-types";
import { PrepareRevealRequest } from "@/features/bitcoin/lib/zod-types/reveal-types";
import { BroadcastRevealRequest } from "@/features/bitcoin/lib/zod-types/broadcast-reveal";
...
export function useMint() {
  ...
  const [walletProvider, setWalletProvider] = useState<LaserEyesWallet | null>(null);
  ...
  const signCommitTransaction = async () => {
    ...
    const { paymentAddress, ordinalAddress, publicKey, paymentPublicKey } = walletProvider;
    ...
    try {
      ...
      const inputsToSign = Array.from({ length: parsedPsbt.data.inputs.length }, (_, index) => ({
        index,
  ...
  const signRevealTransaction = async () => {
    if (
      ...
      handleError(new Error("Wallet or address details missing."), "Configuration Error");
      return;
    }
    ...
    try {
      ...
      const availableAddresses = [walletProvider.ordinalAddress, walletProvider.paymentAddress];
