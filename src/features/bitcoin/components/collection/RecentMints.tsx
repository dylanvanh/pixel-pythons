...
export function RecentMints({
  ...
  recentMints: { inscription_id: string; created_at: string }[] | null;
}) {
  return (
    <div className="w-full mt-10">
      <div className="flex justify-between items-center mb-4">
        ...
        <Link
          to="/bitcoin/collection"
          className="text-sm font-bold underline underline-offset-4 hover:bg-blue-300 px-2 py-1"
        >
          View All
        </Link>
      </div>
      {recentMints === null && <p>Recent Bitcoin mints could not be loaded. Try again later.</p>}
