...
import { useMint } from "@/features/bitcoin/store/mint-store";
import { useLaserEyes, XVERSE, LEATHER } from "@omnisat/lasereyes";
...
export function MintForm() {
  ...
  const renderStepContent = () => {
    switch (mintStep) {
        ...
        return (
          <>
            <ConnectWalletButton
              ...
              openWalletSelector={() => setIsWalletSelectorOpen(true)}
            />
      ...
      case "success":
        return <SuccessStep transactions={transactions} resetMintProcess={resetMintProcess} />;
    }
  };
      ...
      {mintStep === "ready" ? (
        ...
        <>
          <div className="grid auto-rows-min items-start gap-1.5 border-b-4 border-black bg-blue-400 px-6">
            <div className="text-center text-2xl font-bold leading-none">Mint Your Ordinal</div>
          </div>
          <div className="p-6">{renderStepContent()}</div>
        </>
      )}
